# -*- coding: utf-8 -*-
"""
Created on Wed Mar 29 22:57:55 2023
...
if __name__=="__main__":
    ...
    resultrootfolderdir=root+os.sep+r"result\raster\wind_solar_opt_PD"
    cufile=root+os.sep+r"code\Python\src\calcWSOPTRatioPD.cu"
    ...
    func = mod.get_function("calcWSOPTRatioPD")
...
#%%
            ...
            #check if finished
            dirto=resultrootfolderdir+os.sep+power_name+"_no_season"+os.sep+period+os.sep+modelName
            rasfilename=power_name+"_opt_pd_"+str(year_start)+"_"+str(year_end)+".tif" 
...
#%%            
            pd_opt=np.zeros([num_lat,num_lon],dtype=np.float32)
            ...
            for i_batch_lat in range(num_batch_lat):
                for i_batch_lon in range(num_batch_lon):
                    ...
                    ws_opt_batch=np.zeros([num_lat_batch,num_lon_batch],dtype=np.float32)   
                    pd_opt_batch=np.zeros([num_lat_batch,num_lon_batch],dtype=np.float32)    
...
#%%            
                    ...
                    func(cuda.InOut(ws_opt_batch), cuda.InOut(pd_opt_batch), cuda.In(power_pv_batch), cuda.In(power_pv_batch), cuda.In(ws_ratios), \
                          np.int32(num_ws_ratios), np.int32(num_time), np.int32(num_lat_batch), np.int32(num_lon_batch),block=blockSize,grid=gridSize)
                    cuda.Context.synchronize()
                    
                    pd_opt[i_batch_y_start:i_batch_y_end,i_batch_x_start:i_batch_x_end]=pd_opt_batch
                    
                    t2=time.time()
                    print("batch computing finished.    time = %d s.  "%(t2-t1))            
...
#%%
            ...
            if bool_mask_land:
                ...
                pd_opt[land_area_mask==0]=-9999
            
            #the original result is flipped
            pd_opt=np.flipud(pd_opt)         
            pd_opt_sft=np.zeros([num_lat,num_lon],dtype=np.float32)
            pd_opt_sft[:,0:int(num_lon/2)]=pd_opt[:,int(num_lon/2):num_lon]
            pd_opt_sft[:,int(num_lon/2):num_lon]=pd_opt[:,0:int(num_lon/2)]            
            ...
            rasfilename=power_name+"_opt_pd_"+str(year_start)+"_"+str(year_end)+".tif" 
            rasfiledir=dirto+os.sep+rasfilename  
            ptf.writeNumpyToTiff(pd_opt_sft,driver,geotrans,proj,num_lat,num_lon,-9999,rasfiledir,datatype='Float32')
            print("done.\n")
    print("ALL DONE.\n")
